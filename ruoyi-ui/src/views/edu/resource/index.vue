<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd" v-hasPermi="['edu:resource:add']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" plain icon="el-icon-edit" size="mini" :disabled="single" @click="handleUpdate" v-hasPermi="['edu:resource:edit']">修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain icon="el-icon-delete" size="mini" :disabled="multiple" @click="handleDelete" v-hasPermi="['edu:resource:remove']">删除</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="resourceList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="推荐" width="90" align="center">
        <template slot-scope="scope">
          <el-tag size="mini" :type="scope.row.recommendFlag === '1' ? 'warning' : 'info'">{{ scope.row.recommendFlag === '1' ? '已推荐' : '未推荐' }}</el-tag>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="760px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="资源分类" prop="categoryName">
              <treeselect v-model="form.categoryName" :options="categoryOptions" :normalizer="categoryNormalizer" placeholder="请选择资源分类" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="所属课程" prop="courseName">
              <el-select v-model="form.courseName" placeholder="请选择所属课程" style="width: 100%" filterable clearable>
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="资源标签">
              <el-select v-model="form.tagNameList" multiple collapse-tags placeholder="请选择标签" style="width: 100%">
                <el-option v-for="item in tagOptions" :key="item.tagId" :label="item.tagName" :value="item.tagName" />
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="版本号">
              <el-input v-model="form.resourceVersion" placeholder="如 v1.0" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="文件大小(MB)">
              <el-input-number v-model="form.fileSizeMb" :precision="2" :step="0.1" :min="0" style="width: 100%" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
        ...
        <el-form-item label="资源简介">
          <el-input v-model="form.resourceDesc" type="textarea" :rows="4" placeholder="请输入资源简介" />
        </el-form-item>
    ...
    <el-dialog title="资源评论" :visible.sync="commentOpen" width="760px" append-to-body>
      <div v-if="currentResource" class="mb12">
        <el-alert :title="'当前资源：' + currentResource.resourceTitle" type="info" :closable="false" show-icon />
      </div>
      <div v-if="replyTarget" class="reply-banner">
        <span>正在回复：{{ replyTarget.userName }} - {{ replyTarget.content }}</span>
      ...
      <el-form ref="commentForm" :model="commentForm" :rules="commentRules" label-width="80px">
        <el-form-item label="评论内容" prop="content">
          <el-input v-model="commentForm.content" type="textarea" :rows="3" maxlength="500" show-word-limit placeholder="请输入评论内容" />
        </el-form-item>
      </el-form>
        ...
        <div v-for="item in commentList" :key="item.commentId" class="comment-item">
          <div class="comment-header">
            ...
            <div class="comment-actions">
              ...
              <el-button type="text" size="mini" @click="handleCommentReport(item)">举报评论</el-button>
            </div>
          </div>
            ...
            <div v-for="child in item.children" :key="child.commentId" class="comment-child-item">
              <div class="comment-header">
                ...
                <div class="comment-actions">
                  ...
                  <el-button type="text" size="mini" @click="handleCommentReport(child)">举报评论</el-button>
                </div>
              </div>
              <div class="comment-content">
                <span v-if="child.parentUserName" class="reply-user">回复 {{ child.parentUserName }}：</span>{{ child.content }}
              </div>
            </div>
          </div>
        </div>
    ...
    <el-dialog :title="reportDialogTitle" :visible.sync="reportOpen" width="520px" append-to-body>
      <el-form ref="reportForm" :model="reportForm" :rules="reportRules" label-width="90px">
        <el-form-item label="举报对象">
          <el-input :value="reportForm.targetTitle" disabled />
        </el-form-item>
        ...
        <el-form-item label="举报说明" prop="reportContent">
          <el-input v-model="reportForm.reportContent" type="textarea" :rows="4" maxlength="500" show-word-limit placeholder="请输入举报说明" />
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
...
export default {
  ...
  data() {
    return {
      ...
      reportReasonOptions: [
        { label: '内容有误', value: 'content_error' },
        { label: '版权问题', value: 'copyright' },
        { label: '违规违法', value: 'illegal' },
      ...
      reportRules: {
        reasonType: [{ required: true, message: '举报类型不能为空', trigger: 'change' }],
        reportContent: [{ required: true, message: '举报说明不能为空', trigger: 'blur' }]
      }
    }
  },
  computed: {
    reportDialogTitle() {
      return this.reportForm.reportType === 'comment' ? '举报评论' : '举报资源'
    }
  },
  created() {
    this.getList()
  ...
  methods: {
    ...
    statusType(value, type) {
      if (type === 'audit') {
        return { draft: 'info', pending: 'warning', approved: 'success', rejected: 'danger', offline: '' }[value] || 'info'
      }
      return { draft: 'info', review: 'warning', published: 'success', rejected: 'danger', offline: '' }[value] || 'info'
    },
    reset() {
      this.form = {
        resourceId: undefined,
    ...
    resetCommentForm() {
      this.commentForm = {
        resourceId: this.currentResource ? this.currentResource.resourceId : undefined,
        content: '',
        parentId: 0
      }
    ...
    resetReportForm() {
      this.reportForm = {
        reportType: 'resource',
        targetId: this.currentResource ? this.currentResource.resourceId : undefined,
        targetTitle: this.currentResource ? this.currentResource.resourceTitle : '',
        reasonType: undefined,
        reportContent: ''
      }
    ...
    handleUpdate(row) {
      ...
      getResource(resourceId).then(response => {
        ...
        this.form.tagNameList = this.form.tagNames ? this.form.tagNames.split(',').filter(Boolean) : []
        this.open = true
        this.title = '修改资源'
      })
    },
    submitForm() {
      this.$refs.form.validate(valid => {
        ...
        request.then(() => {
          this.msgSuccess(this.form.resourceId ? '修改成功' : '新增成功')
          this.open = false
          this.getList()
        })
      })
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除选中的资源？', '提示', { type: 'warning' }).then(() => {
        return Promise.all(resourceIds.map(id => delResource(id)))
      }).then(() => {
    ...
    handlePreview(row) {
      if (!row.fileUrl) {
        this.msgError('当前资源未配置文件地址')
        return
      }
      window.open(row.fileUrl, '_blank')
    },
    handleSubmit(row) {
      submitResource(row.resourceId).then(() => {
        this.msgSuccess('已提交审核')
        this.getList()
      })
    },
    handleRecommend(row) {
      const recommendFlag = row.recommendFlag === '1' ? '0' : '1'
      recommendResource(row.resourceId, recommendFlag).then(() => {
        this.msgSuccess(recommendFlag === '1' ? '已设为推荐' : '已取消推荐')
        this.getList()
      })
    },
    handleDownload(row) {
      downloadResource(row.resourceId).then(() => {
        this.msgSuccess('下载记录已登记')
        if (row.fileUrl) {
          window.open(row.fileUrl, '_blank')
        }
        this.getList()
    ...
    handleFavorite(row) {
      toggleFavorite(row.resourceId).then(response => {
        this.msgSuccess((response.data && response.data.message) || '操作成功')
        this.getList()
      })
    },
    handleLike(row) {
      toggleLike(row.resourceId).then(response => {
        this.msgSuccess((response.data && response.data.message) || '操作成功')
        this.getList()
      })
    },
    handleComment(row) {
      ...
      this.loadComments(row.resourceId)
    },
    loadComments(resourceId) {
      this.commentLoading = true
      listResourceComments(resourceId).then(response => {
    ...
    handleReply(comment) {
      ...
      this.commentForm.parentId = comment.commentId
    },
    cancelReply() {
      this.replyTarget = null
    ...
    submitComment() {
      this.$refs.commentForm.validate(valid => {
        ...
        addComment(this.commentForm).then(() => {
          this.msgSuccess(this.replyTarget ? '回复已提交，等待审核' : '评论已提交，等待审核')
          const resourceId = this.commentForm.resourceId
          this.resetCommentForm()
    ...
    handleCommentReport(comment) {
      ...
      this.reportForm = {
        ...
        targetTitle: `评论：${comment.userName || '匿名用户'} - ${comment.content}`,
        reasonType: undefined,
        reportContent: ''
      }
      this.resetForm('reportForm')
